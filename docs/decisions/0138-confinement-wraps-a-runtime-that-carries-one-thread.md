...
What actually differs is what a launch knows. The builder binds exactly one
root (`SeatbeltProfileInput.boundRoot`) and needs the thread's mode and
execution policy to settle write, exec, and egress rules. OpenCode supplies all
three because it starts one process per connection and refuses to widen that
...
`thread/start`. No single profile is exact for all of them, and a profile wide
enough for all of them is not a boundary. The Claude runtime does spawn per
query with that thread's `cwd`, but the Agent SDK composes the launch and hands
Octant's spawn callback only command, args, cwd, env, and a signal.
...
- Every other rule of 0009 stands for both runtimes: allowlist-sanitized
  ...
  taint.
- A new provider runtime is wrapped. `providerProcessConfinement.test.ts` pins
  the set above: a provider process module that launches without the shared
  builder fails the suite until this record names its file.
...
- The confinement boundary is written where a reader looks for it instead of
  being inferred from which modules happen to import the builder.
- The unwrapped set cannot grow quietly; a fourth runtime has to argue for
  itself in this record before it ships.
