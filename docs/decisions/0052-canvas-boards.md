...
- **A board is the existing `diagram` block, versioned forward.** There is no
  ...
  executes user or agent HTML or JavaScript). Unknown schema still fails
  closed. Expressiveness grows through the typed catalog, not by rendering a
  program.
...
- **Comments are journaled Canvas facts**, not renderer state. Each is
  ...
  resolved state, and threaded replies. Unauthorized or revoked access
  redacts comment bodies fail-closed. Shared snapshots **exclude comments by
  default**.
