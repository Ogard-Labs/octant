...
Code starts approval-gated unless the user explicitly remembers Full access. A Work-to-Code promotion always starts approval-gated, never inheriting Work authority. The composer shows the posture the next turn will run under, defaulting to the thread's. A turn may only narrow what the thread already grants; Plan mode stays read-only and cannot be overridden from the composer. The transcript records the posture each turn actually ran under. Raising a thread to Full access still needs the native confirmation. The confirmation leads with the action, thread title, and branch or checkout state. Project, thread, repository, and checkout identifiers remain in **Show authority details**, alongside the authority digests.
...
Each Code thread owns an Environment tab in the right dock. Open it from the
dock's tab strip or **Add tool**. The top names the repository, its branch, and
its folder. A card under it says whether the checkout has uncommitted changes
(click it to open Review) and offers **Commit or push** and **Create pull
request**. Below that, one list: local servers, subagents (opens the Agents
tool), computer use, pull requests, what the thread delivers, the working folder
(with **Change working folder**), and usage. Switching the active pane shows that pane's Environment instead of
