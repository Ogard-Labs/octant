...
import type {
  CompatibleProjectEntry,
  RootlessCapabilityFacts,
...
import { ROOTLESS_ATTACH_FOLDER_REASON } from "@octant/contracts/rootless-thread";
import {
...
import { App, launchFromLocation, resolveWorkProviderChoice, resolveDraftProject } from "./App";
...
function rootlessCapabilities(): RootlessCapabilityFacts {
  return {
    workspace: "rootless" as const,
    rootBackedTools: {
      availability: "unavailable" as const,
      reason: ROOTLESS_ATTACH_FOLDER_REASON as RootlessCapabilityFacts["rootBackedTools"]["reason"],
    },
  };
}

function acceptedRootlessReceipt(
  ...
  return {
    ...
    turn: {
      ...
      acceptedAt: "2026-07-29T00:00:00.000Z" as never,
      updatedAt: "2026-07-29T00:00:00.000Z" as never,
    },
  };
}
...
function loadingRootlessCodeShellBootstrap(): ShellBootstrap {
  const value = codeShellBootstrap();
  const layout = value.workspace.layouts.code;
  if (layout.kind !== "group") throw new Error("Expected the default Code group.");
  return {
    ...value,
    workspace: {
      ...value.workspace,
      layouts: {
        ...value.workspace.layouts,
        code: {
          ...layout,
          tabs: layout.tabs.map((tab) =>
            tab.id === layout.activeTabId ? ({ ...tab, hostId: "host-b" } as never) : tab,
          ),
        },
      },
    },
...
function rootlessCodeDraftShellBootstrap(): ShellBootstrap {
  const value = bootstrap();
...
describe("App", () => {
  ...
  it("reports no usable Code model instead of starting a turn on a chat-only model", async () => {
    const user = userEvent.setup();
    const rootlessFirstTurnPort: RootlessFirstTurnPort = {
    ...
    render(
      <App
        ...
        rootlessFirstTurnPort={rootlessFirstTurnPort}
        rootlessThreadClient={
          { listThreads: vi.fn(async () => ({ recents: [], all: [], unfiled: [] })) } as never
        }
        shellClient={client(rootlessCodeDraftShellBootstrap())}
      />,
    );
    ...
    expect(rootlessFirstTurnPort.startFirstTurn).not.toHaveBeenCalled();
  });

  it("keeps Computer Use controls visible while a host-qualified thread is still loading", async () => {
    const computerUseClient = computerUseClientWithSession();
    const sessions = await computerUseClient.list();
    const pendingSessions = deferred<Awaited<ReturnType<ComputerUseClient["list"]>>>();
    vi.mocked(computerUseClient.list).mockReturnValue(pendingSessions.promise);
    const rootlessThreadClient = {
      listThreads: vi.fn(() => new Promise<RootlessThreadListResult>(() => undefined)),
      lookupFirstTurn: vi.fn(),
    } as unknown as RootlessThreadClient;
    ...
    render(
      <App
        ...
        rootlessThreadClient={rootlessThreadClient}
        shellClient={client(loadingRootlessCodeShellBootstrap())}
      />,
    );

    expect(await screen.findByRole("heading", { name: "Loading rootless thread" })).toBeVisible();
    await act(async () => pendingSessions.resolve(sessions));
    expect(await screen.findByRole("button", { name: "Stop computer use" })).toBeVisible();
  ...
  it("keeps a rootless first prompt pending until the authoritative receipt opens the exact source tab", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const accepted = deferred<RootlessTurnLookupResult>();
    let command: StartRootlessThreadTurnCommand | undefined;
    const rootlessFirstTurnPort: RootlessFirstTurnPort = {
      startFirstTurn: vi.fn((value) => {
        command = value;
    ...
    const rootlessThreadClient = {
      listThreads: vi.fn(async () => ({ recents: [], all: [], unfiled: [] })),
    };
    ...
    render(
      <App
        ...
        providerClient={providersWithToolModel()}
        rootlessFirstTurnPort={rootlessFirstTurnPort}
        rootlessThreadClient={rootlessThreadClient as never}
        shellClient={shellApi}
      />,
    );

    const prompt = await screen.findByRole("textbox", { name: "First message" });
    await user.type(prompt, "Keep this first prompt");
    await user.click(screen.getByRole("button", { name: "Create thread" }));

    expect(rootlessFirstTurnPort.startFirstTurn).toHaveBeenCalledWith({
    ...
    expect(screen.getByRole("status", { name: "First-turn status" })).toBeVisible();
    expect(prompt).toHaveValue("Keep this first prompt");
    ...
    expect(await screen.findByRole("tab", { name: "Keep this first prompt" })).toBeVisible();
    expect(shellApi.execute).toHaveBeenCalledWith(
      expect.objectContaining({
        operation: expect.objectContaining({
          kind: "open-tab",
  ...
  it("looks up an ambiguous rootless receipt before opening the exact source tab", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const lookup = deferred<RootlessTurnLookupResult>();
    let command: StartRootlessThreadTurnCommand | undefined;
    const rootlessFirstTurnPort: RootlessFirstTurnPort = {
      startFirstTurn: vi.fn(async (value): Promise<RootlessTurnLookupResult> => {
        command = value;
        return {
          kind: "ambiguous",
          ...
          prompt: value.prompt,
          capabilities: rootlessCapabilities(),
          message: "The provider outcome is uncertain.",
          acceptedAt: "2026-07-29T00:00:00.000Z" as never,
          updatedAt: "2026-07-29T00:00:00.000Z" as never,
        };
      }),
      lookupFirstTurn: vi.fn(() => lookup.promise),
      cancelFirstTurn: vi.fn(),
    };
    ...
    await user.type(await screen.findByRole("textbox", { name: "First message" }), "Recover me");
    await user.click(screen.getByRole("button", { name: "Create thread" }));

    expect(await screen.findByRole("status", { name: "First-turn status" })).toHaveTextContent(
      "The provider outcome is uncertain. Checking the authoritative receipt",
    );
    await act(async () => {
      lookup.resolve({
        kind: "accepted",
    ...
    expect(await screen.findByRole("tab", { name: "Recover me" })).toBeVisible();
    await waitFor(() =>
      expect(rootlessFirstTurnPort.lookupFirstTurn).toHaveBeenCalledWith(command!.requestId),
    );
    expect(shellApi.execute).toHaveBeenCalledWith(
      expect.objectContaining({
        operation: expect.objectContaining({
          kind: "open-tab",
          tab: expect.objectContaining({ hostId: "local", threadId: command!.threadId }),
        }),
      }),
    );
  });

  it("preserves the rootless draft and offers retry when no first turn was created", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const availableProjects = projectBootstrap();
    const rootlessFirstTurnPort: RootlessFirstTurnPort = {
      startFirstTurn: vi.fn(
        async (command): Promise<RootlessTurnLookupResult> => ({
          kind: "not-created",
          requestId: command.requestId,
    ...
    render(
      <App
        ...
        projectClient={projects({
          ...availableProjects,
          availability: [{ ...availableProjects.availability[0]!, status: "available" as const }],
        })}
        projectWindowCapability={projectWindowCapability}
        providerClient={providersWithToolModel()}
        rootlessFirstTurnPort={rootlessFirstTurnPort}
    ...
    const prompt = await screen.findByRole("textbox", { name: "First message" });
    await user.type(prompt, "Do not lose this draft");
    await user.click(screen.getByRole("button", { name: "Create thread" }));

    expect(await screen.findByRole("alert")).toHaveTextContent(
      "The provider is unavailable. Retry or attach a folder.",
    );
    expect(prompt).toHaveValue("Do not lose this draft");
    expect(screen.getByRole("button", { name: "Retry creating thread" })).toBeVisible();
    expect(shellApi.execute).not.toHaveBeenCalledWith(
      expect.objectContaining({ operation: expect.objectContaining({ kind: "open-tab" }) }),
    );

    await user.click(screen.getByRole("button", { name: "New thread in Octant" }));

    await waitFor(() => expect(screen.queryByRole("alert")).not.toBeInTheDocument());
    expect(screen.getByRole("button", { name: "Folder: Octant" })).toBeVisible();
  });

  it("settles local navigation when cancellation races before server acceptance", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const pending = deferred<RootlessTurnLookupResult>();
    let command: StartRootlessThreadTurnCommand | undefined;
    const rootlessFirstTurnPort: RootlessFirstTurnPort = {
      startFirstTurn: vi.fn((value) => {
        command = value;
      ...
      lookupFirstTurn: vi.fn(),
      cancelFirstTurn: vi.fn(async () => {
        throw new Error("No accepted turn was found.");
      }),
    };
    ...
    const prompt = await screen.findByRole("textbox", { name: "First message" });
    await user.type(prompt, "Keep this cancellable draft");
    await user.click(screen.getByRole("button", { name: "Create thread" }));
    await user.click(await screen.findByRole("button", { name: "Cancel first turn" }));

    expect(rootlessFirstTurnPort.cancelFirstTurn).toHaveBeenCalledWith({
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "No accepted turn was found. The local start was stopped.",
    );
    expect(prompt).toHaveValue("Keep this cancellable draft");
    expect(screen.getByRole("button", { name: "Retry creating thread" })).toBeEnabled();

    await act(async () => {
      pending.resolve(acceptedRootlessReceipt(command!));
    });
    expect(screen.queryByRole("tab", { name: "Keep this cancellable draft" })).toBeNull();
  });

  it("uses a fresh lookup when cancellation finds an already-terminal first turn", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const pending = deferred<RootlessTurnLookupResult>();
    let command: StartRootlessThreadTurnCommand | undefined;
    const rootlessFirstTurnPort: RootlessFirstTurnPort = {
      ...
      lookupFirstTurn: vi.fn(async () => acceptedRootlessReceipt(command!)),
      cancelFirstTurn: vi.fn(async (value) => ({
        ...value,
    ...
    await user.type(await screen.findByRole("textbox", { name: "First message" }), "Lookup only");
    await user.click(screen.getByRole("button", { name: "Create thread" }));
    await user.click(await screen.findByRole("button", { name: "Cancel first turn" }));
    ...
    expect(await screen.findByRole("tab", { name: "Lookup only" })).toBeVisible();
    expect(shellApi.execute).toHaveBeenCalledWith(
      expect.objectContaining({
        operation: expect.objectContaining({
          kind: "open-tab",
  ...
  it("opens duplicate rootless titles by the selected source-qualified identity", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const codeApi = codes();
    const thread = {
    ...
    const local = { ...thread, hostId: "local" };
    const remote = { ...thread, hostId: "host-b", modelId: "claude-sonnet" };
    const rootlessThreadClient = {
      listThreads: vi.fn(async () => ({
        recents: [local],
        all: [local, remote],
        unfiled: [local, remote],
      })),
    };
    ...
    await waitFor(() => expect(rootlessThreadClient.listThreads).toHaveBeenCalledOnce());
    await waitFor(() => expect(screen.queryByText("Loading threads…")).not.toBeInTheDocument());
    const duplicates = screen.getAllByRole("button", { name: /Duplicate title/ });
    ...
    expect(await screen.findByRole("region", { name: "Rootless Code thread" })).toHaveTextContent(
      "No folder",
    );
    expect(screen.getByText("Root-backed tools are unavailable")).toBeVisible();
    ...
    expect(codeApi.thread).not.toHaveBeenCalled();
  });

  it("fails closed for non-local rootless thread attach and causes no lookup/browse/select/attach side effects", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const now = "2026-07-29T00:00:00.000Z" as never;
    const thread: RootlessThreadSummary = {
      threadId: "00000000-0000-4000-8000-000000000902" as never,
      title: "Duplicate title",
      mode: "code" as never,
      hostId: "host-b" as never,
      providerInstanceId: "90000000-0000-4000-8000-000000000001" as never,
      modelId: "claude-sonnet" as never,
      workspaceKind: "rootless" as never,
      createdAt: now,
      updatedAt: now,
    };
    ...
    const lookupCompatibleProjects = vi.fn(async () => []);
    const attachFolder = vi.fn(async () => ({
      kind: "attached" as const,
      attachmentId: "00000000-0000-4000-8000-000000000100" as never,
      threadId: thread.threadId,
      projectId: "00000000-0000-4000-8000-000000000020" as never,
      attachedAt: now,
    }));
    const rootlessThreadClient: RootlessThreadClient = {
      listThreads,
    ...
    try {
      ...
      await waitFor(() => expect(screen.queryByText("Loading threads…")).not.toBeInTheDocument());
      await user.click(await screen.findByRole("button", { name: /Duplicate title/ }));

      const workspace = await screen.findByRole("region", { name: "Rootless Code thread" });
      await user.click(within(workspace).getByRole("button", { name: "Attach folder" }));

      expect(
        await screen.findByText(/Attach a folder from the host where this thread was created/),
      ).toBeVisible();
      expect(lookupCompatibleProjects).not.toHaveBeenCalled();
      expect(attachFolder).not.toHaveBeenCalled();
      expect(fetchMock).not.toHaveBeenCalledWith(
        expect.stringContaining("/api/folders/browse"),
        expect.anything(),
      );
      expect(fetchMock).not.toHaveBeenCalledWith(
        expect.stringContaining("/api/folders/select"),
        expect.anything(),
      );
  ...
  it("attaches a folder to a local rootless Code thread through the web folder picker and refreshes the list", async () => {
    const user = userEvent.setup();
    const shellApi = client(rootlessCodeDraftShellBootstrap());
    const compatibleProject: CompatibleProjectEntry = {
      projectId: "00000000-0000-4000-8000-000000000020" as never,
      displayName: "Attached Docs" as never,
      rootPath: "/Users/example/Documents/attached-docs" as never,
    };
    const now = "2026-07-29T00:00:00.000Z" as never;
    const thread: RootlessThreadSummary = {
      threadId: "00000000-0000-4000-8000-000000000902" as never,
    ...
    let listThreadsCallCount = 0;
    const listThreads = vi.fn(async (): Promise<RootlessThreadListResult> => {
      listThreadsCallCount++;
      if (listThreadsCallCount === 1) return { recents: [], all: [thread], unfiled: [thread] };
      return { recents: [attached], all: [attached], unfiled: [] };
    });
    const lookupCompatibleProjects = vi.fn(async () => [compatibleProject]);
    const attachFolder = vi.fn(async () => ({
      kind: "attached" as const,
      attachmentId: "00000000-0000-4000-8000-000000000100" as never,
      threadId: thread.threadId,
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, _init?: RequestInit) => {
      const url = typeof input === "string" ? input : input.toString();
      if (url.endsWith("/api/folders/browse")) {
        return new Response(
          JSON.stringify({
            candidates: [
              {
                candidateId: "00000000-0000-4000-8000-000000000300",
                displayName: "attached-docs",
      ...
      if (url.endsWith("/api/folders/select")) {
        return new Response(
          JSON.stringify({
            receiptId: "receipt-web-local",
            displayName: "attached-docs",
            selectedAt: now,
          }),
          { status: 200, headers: { "content-type": "application/json" } },
    ...
    try {
      ...
      await waitFor(() =>
        expect(lookupCompatibleProjects).toHaveBeenCalledWith({
          threadId: thread.threadId,
          mode: thread.mode,
          hostId: "local",
        }),
      ...
      await waitFor(() =>
        expect(fetchMock).toHaveBeenCalledWith(
          expect.stringContaining("/api/folders/browse"),
          expect.objectContaining({
            body: expect.stringContaining('"hostId":"local"'),
          }),
        ),
      );
      ...
      expect(await screen.findByText(/Attached to/)).toBeVisible();
    } finally {
      vi.unstubAllGlobals();
    }
  });
  ...
  it("opens an attached rootless Code thread carried by an Electron Project-window target", async () => {
    const shellApi = client(bootstrap());
    const rootlessThreadId = "10000000-0000-4000-8000-000000000099";
    const hostBridge: OctantHostBridge = {
      ...credentialHostOperations(),
      close: vi.fn(),
      initialProjectTarget: {
      ...
      expect(
        shellApi.execute.mock.calls.some(
          ([command]) =>
            ...
            command.operation.kind === "open-tab" &&
            command.operation.tab.kind === "code-overview" &&
            String(command.operation.tab.threadId) === rootlessThreadId &&
            command.operation.tab.hostId === "local",
        ),
      ).toBe(true),
    );
  });
