...
export interface GoalRouteDependencies {
   ...
   * renderer could read another thread's Goal or journal state for a thread
   * that does not exist. Required rather than optional: a host that cannot
   * authorize must not serve Goals at all.
   */
...
export function createGoalRouteHandler(dependencies: GoalRouteDependencies) {
  const now = dependencies.now ?? Date.now;
  return async (request: Request): Promise<Response | undefined> => {
    const url = new URL(request.url);
    if (!url.pathname.startsWith("/api/goals")) return undefined;
    const origin = request.headers.get("origin");
    if (!isLoopbackHostname(url.hostname)) {
      return json({ error: "Goal API requests must use loopback." }, 400, null);
    }
    ...
    try {
      ...
      if (error instanceof WindowAuthorityError) {
        return json({ error: "Goal request is unauthorized." }, 401, origin);
      }
      return json({ error: "Goal request is invalid." }, 400, origin);
    }
    ...
    if (request.method === "GET" && url.pathname === "/api/goals") {
      ...
      if (!(await dependencies.authorizeThread({ threadId, windowId }))) {
        return json({ error: "Goal request is not authorized for this thread." }, 403, origin);
      }
      return json(dependencies.service.read(threadId), 200, origin);
    }

    if (request.method === "POST" && url.pathname === "/api/goals/commands") {
      ...
      try {
        ...
        return json({ error: "Goal command failed." }, 500, origin);
      }
    }
...
/** The thread a Goal command names, or undefined when the body cannot name one. */
