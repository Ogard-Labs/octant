...
const enabled = process.env.OCTANT_KIMI_SMOKE === "1";
const binaryPath = process.env.OCTANT_KIMI_BINARY ?? "/opt/homebrew/bin/kimi";
const kimiHome = process.env.OCTANT_KIMI_HOME ?? "/tmp/octant-kimi-smoke";
const instanceId = decodeProviderInstanceId("80000000-0000-4000-8000-000000000331");
const completionSessionId = decodeProviderSessionId("80000000-0000-4000-8000-000000000332");
const interruptSessionId = decodeProviderSessionId("80000000-0000-4000-8000-000000000333");

describe("installed Kimi Code runtime", () => {
  it.skipIf(!enabled)(
    "uses provider-native authentication for a harmless stream, exact resume, interrupt, and cleanup",
    async () => {
      const temporaryRoot = await realpath(await mkdtemp(join(tmpdir(), "octant-kimi-smoke-")));
      const registry = new ProviderRuntimeRegistry();
      const driver = makeAcpDriver({
        profile: acpProviderProfiles["kimi-code"],
        instanceId,
        binaryPath,
        managedHome: kimiHome,
          ...
          Effect.scoped(
            Effect.gen(function* () {
              ...
              yield* connection.send({
                sessionId: completionSessionId,
                prompt: "Reply with exactly: octant-kimi-smoke",
              ...
              if (started.resumeCursor === undefined)
                throw new Error("Kimi resume cursor missing.");
