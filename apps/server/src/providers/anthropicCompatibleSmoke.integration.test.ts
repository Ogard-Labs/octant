...
const smoke = process.env.OCTANT_ANTHROPIC_COMPATIBLE_SMOKE === "1" ? it : it.skip;

/**
 * Real Anthropic-compatible endpoint smoke.
 *
 * Opt-in: set OCTANT_ANTHROPIC_COMPATIBLE_SMOKE=1 plus the required
 * environment variables to exercise a real Anthropic-compatible Messages
 * endpoint. When credentials are absent, this smoke is skipped — missing
 * credentials remain an explicit non-passing gate, never a false pass.
 */
describe("real Anthropic-compatible endpoint", () => {
  smoke(
    "discovers models and completes a Messages stream without production env fallback",
    async () => {
      ...
      const cancelSessionId = decodeProviderSessionId("80000000-0000-4000-8000-000000000903");
        ...
        Effect.scoped(
          Effect.gen(function* () {
            ...
            yield* connection.send({
              sessionId,
              prompt: "Reply with exactly: octant-anthropic-smoke",
              attachments: [],
              tools: [],
            });
...
function required(name: string): string {
  ...
  if (value === undefined || value.trim().length === 0) {
    throw new Error(
      `Anthropic-compatible smoke requires ${name}; no credential values are logged.`,
    );
  }
  return value;
}
