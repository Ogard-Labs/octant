...
export interface AnthropicHttpLimits {
...
export interface AnthropicCompatibleEndpoint {
  ...
  readonly fetch: AnthropicCompatibleFetch;
  readonly credentialResolver?: ProviderCredentialResolver;
  readonly limits: AnthropicHttpLimits;
  readonly url: (path: "models" | "messages") => string;
}

export type AnthropicCompatibleFetch = (
...
export interface AnthropicGenerationRequest {
  ...
  readonly onRejected?: (metadata: AnthropicGenerationRejection) => void;
}

export interface AnthropicGenerationRejection {
  readonly httpStatus: number;
  readonly failure: ProviderFailure;
}

export function makeAnthropicCompatibleEndpoint(
  options: AnthropicCompatibleEndpointOptions,
  ...
  return Object.freeze({
    ...
    limits,
    url: (path: "models" | "messages") => `${baseUrl}/${path}`,
  });
}
...
export async function probeAnthropicModels(
  ...
  if (!response.ok) {
    const failure = classifyAnthropicHttpFailure(response);
  ...
  const discoveredIds = parseAnthropicModelList(value);
  const discovered = new Set(discoveredIds);
  const models: ProviderModel[] = [...discovered].map(discoveredModel);
  for (const id of endpoint.configuration.manualModelIds) {
    if (!discovered.has(id)) models.push(manualModel(id));
  }
  return { readiness: "ready", models };
}

export async function requestAnthropicGeneration(
  endpoint: AnthropicCompatibleEndpoint,
  request: AnthropicGenerationRequest,
): Promise<Response> {
  ...
  if (!response.ok) {
    let failure = classifyAnthropicHttpFailure(response);
    ...
    failure = sanitizeAnthropicFailure(failure);
    try {
...
export function classifyAnthropicHttpFailure(
  response: Response,
  now = Date.now(),
): ProviderFailure {
...
export function markAnthropicModelVerified(
...
async function performRequest(
  ...
  try {
    ...
    headers.set("anthropic-version", endpoint.configuration.protocolVersion);
    if (endpoint.configuration.authentication !== "none") {
      ...
      if (endpoint.configuration.authentication === "api-key") {
...
function parseAnthropicModelList(value: unknown): readonly string[] {
...
function assertPositiveLimit(value: number): void {
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new Error("Anthropic HTTP limits must be positive integers.");
  }
}
...
function sanitizeAnthropicFailure(error: unknown): ProviderFailure {
  try {
